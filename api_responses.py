...
def generate_api_response(url_list: list[str], as_file=True) -> ScraperResponseModel:
    ...
    if processed_data['scraped']:
        if as_file:
            ...
            csv_output = CSVGenerator.generate_csv_string_from_data(processed_data['scraped'])
            attachment_data = AttachmentModel(filename="scraped_data.csv", content_type="text/csv", data=csv_output)
        csv_generated = True

    if csv_generated and not any(
            [processed_data['invalid_urls'], processed_data['connection_errors'], processed_data['scraping_errors'],
             processed_data['url_not_found_errors']]):
        return ScraperResponseModel(
            status="SUCCESS",
    ...
    if csv_generated:
        return ScraperResponseModel(
            ...
            message="Some URLs resulted in connection or scraping errors. CSV Generated containing the successfully "
                    "scraped products.",
            data=DataModel(attachment=attachment_data, processed_urls_dict=ProcessedUrlsDictModel(**processed_data))
        )
